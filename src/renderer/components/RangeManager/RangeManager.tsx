...
export function RangeManager() {
  ...
  const availableCategories = customCategories.length > 0 ? customCategories : ['Mes ranges'];

  return (
    <div className="flex flex-col gap-3">
      {/* Header actions */}
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Bibliothèque de ranges</span>
      </div>
      ...
      {showNewCategory && (
        <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-3 space-y-2">
          <span className="text-[10px] text-zinc-500 uppercase tracking-wider">Nouvelle catégorie</span>
      ...
      {showNewRange && (
        <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-3 space-y-2">
          <span className="text-[10px] text-zinc-500 uppercase tracking-wider">Nouveau range</span>
          <input
            type="text"
            value={newRangeName}
            onChange={(e) => setNewRangeName(e.target.value)}
            placeholder="Nom (ex: Mon open UTG)"
          ...
          <input
            ...
            placeholder="Notation (ex: TT+, AQs+, AKo)"
          ...
          <div className="flex gap-2">
            <button onClick={handleCreateRange} disabled={!newRangeName.trim() || !newRangeNotation.trim() || !newRangeCategory} className="px-3 py-1.5 rounded text-xs font-semibold bg-emerald-600 text-white hover:bg-emerald-500 disabled:opacity-40">Créer</button>
            <button onClick={() => setShowNewRange(false)} className="px-2 py-1.5 rounded text-xs text-zinc-500 hover:text-zinc-300">Annuler</button>
          </div>
        </div>
      )}
          ...
          return (
            <div key={cat.name} className={`rounded-lg border transition-colors ${isDisabled ? 'border-zinc-800 opacity-50' : 'border-zinc-700'}`}>
              ...
              <div className="flex items-center gap-2 px-3 py-2">
                ...
                {!isBuiltIn && (
                  <button
                    onClick={() => { if (confirm(`Supprimer "${cat.name}" et tous ses ranges ?`)) deleteCustomCategory(cat.name); }}
              ...
              {isExpanded && (
                <div className="border-t border-zinc-800">
                  {cat.presets.length === 0 && (
                    <div className="px-5 py-2 text-[10px] text-zinc-600">Aucun range</div>
                  )}
                      ...
                      return (
                        <div key={pi} className="px-4 py-2 space-y-1 bg-zinc-900/50">
                          ...
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setEditingId(null)} className="px-2 py-0.5 rounded text-[10px] text-zinc-500">Annuler</button>
                          </div>
                        </div>
                      );
                    }
